...
void InitializeNewDevice(sPoKeysDevice* device)
{
    ...
    device->AnalogInput = (sPoKeysAnalogData*)hal_malloc(sizeof(sPoKeysAnalogData) * device->info.iAnalogInputCount);
    memset(device->AnalogInput, 0, sizeof(sPoKeysAnalogData) * device->info.iAnalogInputCount);
...
{
    *(device->Pins[pin].DigitalValueSet.out) = value;
}

uint8_t PK_SL_DigitalInputGet(sPoKeysDevice* device, uint8_t pin)
{
    return *(device->Pins[pin].DigitalValueGet.in);
}
