...
**6. Report exact evidence.** Before claiming completion, report: files changed, requirement addressed, behavior implemented, tests added or changed, exact build and test commands executed, results of those commands, hardware or RT checks performed, checks not performed and why, remaining risks. Use precise terms: **Implemented** (production behavior exists); **Compiled** (compilation only); **Tested** (named tests executed and passed); **RT-validated** (tested in LinuxCNC RT environment); **Timing-validated** (measured with an identified method and threshold). For hardware/HIL evidence, use **HIL-observed**, **HIL-test-executed**, and **HIL-verified** as defined in `.github/skills/hil-tdd/references/result-schema.md`. Do not use "complete," "working," "fixed," "RT-safe," or "verified" without corresponding evidence.
