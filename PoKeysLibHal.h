...
/** 
 * @brief PoKeys device information (HAL compatible version)
 */
typedef struct
{
    hal_u32_t iPinCount;                        ///< Number of pins, physically on the device
    hal_u32_t iPWMCount;                        ///< Number of pins that support PWM output
    hal_u32_t iBasicEncoderCount;               ///< Number of basic encoders
    hal_u32_t iEncodersCount;                   ///< Number of encoder slots available
    hal_u32_t iFastEncoders;                    ///< Number of fast encoders supported
    hal_u32_t iUltraFastEncoders;               ///< Number of ultra fast encoders
    hal_u32_t PWMinternalFrequency;             ///< Main PWM peripheral clock
    hal_u32_t iAnalogInputs;                    ///< Number of available analog inputs

    hal_u32_t iKeyMapping;                      ///< Device supports key mapping (acts as a USB keyboard)
    hal_u32_t iTriggeredKeyMapping;             ///< Device supports triggered key mapping
    hal_u32_t iKeyRepeatDelay;                  ///< User customizable key repeat rates and delays
    hal_u32_t iDigitalCounters;                 ///< Digital counters support
    hal_u32_t iJoystickButtonAxisMapping;       ///< Joystick button mapping support
    hal_u32_t iJoystickAnalogToDigitalMapping;  ///< Analog to digital key mapping support
    hal_u32_t iMacros;                          ///< Customizable macro sequences
    hal_u32_t iMatrixKeyboard;                  ///< Matrix keyboard support
    hal_u32_t iMatrixKeyboardTriggeredMapping;  ///< Matrix keyboard triggered key mapping
    hal_u32_t iLCD;                             ///< Alphanumeric LCD display support
    hal_u32_t iMatrixLED;                       ///< Matrix LED display support
    hal_u32_t iConnectionSignal;                ///< Connection signal output support
    hal_u32_t iPoExtBus;                        ///< PoExtBus support
    hal_u32_t iPoNET;                           ///< PoNET bus support
    hal_u32_t iAnalogFiltering;                 ///< Analog inputs low-pass filtering
    hal_u32_t iInitOutputsStart;                ///< Initialize outputs at startup
    hal_u32_t iprotI2C;                         ///< I2C master support
    hal_u32_t iprot1wire;                       ///< 1-wire master support
    hal_u32_t iAdditionalOptions;               ///< Activation key options
    hal_u32_t iLoadStatus;                      ///< Load status reporting
    hal_u32_t iCustomDeviceName;                ///< Custom device name support
    hal_u32_t iPoTLog27support;                 ///< PoTLog27 firmware support
    hal_u32_t iSensorList;                      ///< Sensor list support
    hal_u32_t iWebInterface;                    ///< Web interface support
    hal_u32_t iFailSafeSettings;                ///< Fail-safe mode support
    hal_u32_t iJoystickHATswitch;               ///< Joystick HAT switch mapping
    hal_u32_t iPulseEngine;                     ///< Pulse engine support
...
typedef struct
{
 ...
 hal_s32_t         SoftLimitMaximum[8];       // Soft limit maximum position
 hal_s32_t         SoftLimitMinimum[8];       // Soft limit minimum position
 ...
 hal_s32_t         HomeOffsets[8];            // Home position offset
 ...
 hal_s32_t         ProbePosition[8];          // Position where probe detected change
 hal_s32_t         ProbeMaxPosition[8];       // Maximum position to travel to until stopping and returning error
 hal_float_t            MaxSpeed[8];               // Maximum axis speed (in pulses per ms)
 hal_float_t            MaxAcceleration[8];        // Maximum axis acceleration (in pulses/ms/ms)
 hal_float_t            MaxDecceleration[8];       // Maximum axis deceleration (in pulses/ms/ms)


 hal_s32_t         MPGjogMultiplier[8];       // MPG jog multiplier value
 ...
 hal_u32_t 		HomeBackOffDistance[8];	   // Back-off distance after homing
 ...
 hal_float_t            ReferenceVelocityPV[8];    // Reference velocity in PV mode (between 0.0 and 1.0) as ratio of max. speed
 ...
 // ------ 64-bit region boundary ------
 hal_float_t            ProbeSpeed;                // Probe speed (ratio of the maximum speed)
 hal_float_t  			reservedf;

 hal_u32_t 		DebugValues[16];
 ...
 hal_s32_t			SpindleSpeedEstimate;
 hal_s32_t			SpindlePositionError;
 hal_u32_t 		SpindleRPM;
 hal_u32_t 		spindleIndexCounter;
 ...
 hal_u32_t 		EncoderIndexCount;
 hal_u32_t 		EncoderTicksPerRotation;
 hal_u32_t 		EncoderVelocity;
 hal_u32_t 		reservedEnd;
...
typedef struct
{
 hal_u32_t  DeviceTypeID;                     // ePK_DeviceTypes ID
 hal_u32_t  SerialNumber;                     // Serial number of the device
...
typedef struct
{
 hal_u32_t   PWMperiod;                       // PWM period, shared among all channels
    uint32_t  reserved;
 hal_u32_t  *PWMduty[6];                         // PWM duty cycles (range between 0 and PWM period)
...
typedef struct
{
 hal_u32_t  DataMemorySize;
 hal_u32_t  CodeMemorySize;
 hal_u32_t  Version;
 hal_u32_t  DeviceType;
} sPoILinfo;
...
typedef struct
{
 ...
 hal_u32_t          MasterEnable;
 hal_u32_t          currentTask;
 hal_u32_t          STATUS;
 hal_u32_t          W;
 hal_u32_t          PC;
 hal_u32_t          ExceptionPC;
 hal_u32_t          CoreState;
 hal_u32_t          CoreDebugMode;
 hal_u32_t          CoreDebugBreakpoint;
 hal_u32_t  		 ExternalCoreID;

 hal_u32_t          ExceptionLocation;
 hal_u32_t          reserved_for_64;
 hal_u32_t          ExceptionDescription[4];
...
typedef struct
{
 hal_u32_t  SerialNumber;                                   // Serial number
...
typedef struct
{
 ...
 uint8_t                   reserved;
 ALIGN_TEST(5)
 hal_u32_t  sendRetries;
 hal_u32_t  readRetries;
 hal_u32_t  socketTimeout;
...
#endif
// int32_t PK_EnumerateUSBDevicesAsync(PKUSBEnumerator *ctx); No USB in RTAPI !!!
 ...
 * and the requested bitrate as payload.
 *
 * @param device Target device handle.
...
/**
 * Register a CAN filter entry.
 *
 * Uses PK_CMD_CAN_OPERATIONS subcommand 0x10.  Parameter
 * @p format selects the identifier format while @p CANid is the
 * identifier value.
 *
 * @param device Target device handle.
 * @param format Identifier format (0 = standard, 1 = extended).
 * @param CANid  CAN identifier to match.
 * @return Result code from SendRequest().
 */
POKEYSDECL int32_t PK_CANRegisterFilter(sPoKeysDevice* device, uint8_t format, uint32_t CANid);

/**
 * Transmit a CAN message using subcommand 0x20.
 *
 * @param device Target device handle.
 * @param msg    Pointer to ::sPoKeysCANmsg describing the frame.
 * @return Result code from SendRequest().
 */
 ...
 *
 * The command uses subcommand 0x31 and returns a status byte
 * indicating message availability.  When non‑zero the message
 * is copied into @p msg.
 *
 * @param device Target device handle.
...
/**
 * Flush pending CAN messages (subcommand 0x32).
 *
 * @param device Target device handle.
 * @return Result code from SendRequest().
 */
