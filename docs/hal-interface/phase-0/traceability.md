...
## External Counterpart Boundary

The records in Sections 13 and 14 below describe the external `pokeys_homecomp`
counterpart boundary. `pokeys_homecomp` is a separate, independently loaded
LinuxCNC component (source E-010). It is not a pokeysHal subsystem, not
implemented by pokeysHal, and does not share lifecycle management with
pokeysHal.

- `joint.N.*` pins are created and owned by `pokeys_homecomp` exclusively.
- They are not pokeysHal exports and are not part of the normative Phase 0
